...
# decode the predictions, then apply non-maximum suppression to
...
for (startX, startY, endX, endY) in boxes:
    ...
    text1 = pytesseract.image_to_string(roi, config=config)
    mytext = text1
    language = 'en'
    ...
    # of results
    results.append(((startX, startY, endX, endY), text1))
    ...
    text1 = "".join([c if ord(c) < 128 else "" for c in text1]).strip()
    output = orig.copy()
    ...
    # Draw text
    cv2.putText(output, text1, (startX, startY - 20), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
